...
class GroupViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x, group_token, attn_dict = layer(x, group_token, return_attn=False)
    ...
    def forward_cls_features(self, x):
        group_token = None
        attn_dict_list = []
        for layer in self.layers:
            x, group_token, attn_dict = layer(x, group_token, return_attn=False)
    ...
    def forward(self, x, prompt, patch_info=None):
        x, group_token, attn_dicts = self.forward_cls_features(x)
        # x, group_token, attn_dicts = self.forward_features(x)
        x = self.forward_image_head(x)
        return x
