...
class CrossFrameAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        msg_token = msg_token.view(b, iT, 1, d)
        
        msg_token = msg_token.permute(1,2,0,3).view(iT, b, d)
        msg_token = msg_token + self.drop_path(self.message_attn(self.message_ln(msg_token), self.message_ln(msg_token),
                                                                 self.message_ln(msg_token), need_weights=False)[0])
        msg_token = msg_token.view(iT, 1, b, d).permute(1,2,0,3)
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, droppath=None, use_checkpoint=False, T=8):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[CrossFrameAttentionBlock(width, heads, attn_mask, droppath[i], T) for i in range(layers)])
       
    def forward(self, x: torch.Tensor, bs=None):
        if not self.use_checkpoint:
            return self.resblocks([x, bs])
        else:
