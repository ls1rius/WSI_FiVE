...
class XCLIP(CLIP):
    ...
    def encode_video(self, image, sample_range):
        ...
        video_features = self.mit(cls_features, sample_range)
    ...
    def forward(self, image, text, sample_range):
        b = image.shape[0]
        video_features, img_features = self.encode_video(image, sample_range)
        
        img_features_new = []
        for i in range(len(sample_range)):
            img_features_new.append(img_features[i, :sample_range[i]].sum(dim=0) / sample_range[i])
        img_features_new = torch.stack(img_features_new)
        img_features = img_features_new
        # img_features = img_features.mean(dim=1, keepdim=False)

        # L10 change for finetune position encode
...
def load(model_path, name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", 
    ...
    model = build_model(state_dict or model.state_dict(), T=T, droppath=droppath, 
                        ...
                        context_length=context_length, #add L10
                        )
    if str(device) == "cpu":
        model.float()
