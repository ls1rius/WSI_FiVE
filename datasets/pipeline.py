...
class SampleWSIData:
    ...
    def _get_train_clips(self, num_frames):
        ...
        num_clips = min(int(num_frames * self.num_clips_percent), self.num_clips)
    ...
    def _get_test_clips(self, num_frames):
        ori_clip_len = self.clip_len * self.frame_interval
        ### sample all is the right way, but sample part for test is still work
        # num_clips = num_frames # sample all patches for test
        num_clips = min(int(num_frames * self.num_clips_percent), self.num_clips)
        ###
    ...
    def __call__(self, results):
        ...
        results['num_clips_percent'] = self.num_clips_percent
        ...
        ###
            ...
            img_pth = osp.join(data_prefix, filename).replace('LUAD-patch', 'LUADSC-patch-embed-pub').replace(
                'LUSC-patch', 'LUADSC-patch-embed-pub').replace('/patch/', '/patch-embed-pub/') + '.csv'
        ...
        results['sample_range'] = np.array(sample_range)
        results['original_shape'] = imgs[0].shape[:2]
        results['img_shape'] = imgs[0].shape[:2]
        results['imgs_embed'] = np.array(imgs_embed)
        results['patch_inds'] = np.array(patch_inds)

        ### del untensorable data
        ### fix data
        results['data_prefix'] = None
    ...
    def __repr__(self):
        repr_str = (f'{self.__class__.__name__}('
                    ...
                    f'num_clips_percent={self.num_clips_percent}, '
