...
class VideoDataset(BaseDataset):
    ...
    # change by L10
    # used for wsi csv data
    def load_annotations(self):
        video_infos = []
        df = pd.read_csv(self.ann_file)
        patient_id_sub_list = list(set(df['patient_id_sub']))
        for i in range(len(patient_id_sub_list)):
            video_infos.append(dict(filename=patient_id_sub_list[i],
                                    data_prefix=self.data_prefix,
                                    label=np.array(df[df['patient_id_sub']==patient_id_sub_list[i]].iloc[0]['labels_id']),
                                    patch_maxr=np.array(df[df['patient_id_sub']==patient_id_sub_list[i]].iloc[0]['patch_maxr']),
                                    patch_maxc=np.array(df[df['patient_id_sub']==patient_id_sub_list[i]].iloc[0]['patch_maxc']),
                                    patch_mask=df[df['patient_id_sub']==patient_id_sub_list[i]].iloc[0]['patch_mask'],
                                    ))
        return video_infos
...
def build_dataloader(logger, config):
    ...
    train_pipeline = [
        ...
        dict(type='Collect', keys=['imgs', 'label', 'sample_range', 'posis', 'patch_maxr', 'patch_maxc'], meta_keys=[]),
